...
$m = [ordered]@{
    "Theme Mode:" = @("Follow system switches automatically from the current Windows app theme.", "跟随系统会根据 Windows 当前的应用主题自动切换。")
    "Background Opacity Preset:" = @("Controls transparency and background blur for app panels and dialogs.", "影响应用面板与模态框的透明度和背景模糊程度。")
    ...
    "Double-click Completed Task:" = @("Action to perform when double-clicking a completed download task.", "设置双击已完成下载任务时的行为。")
    "Double-click Uncompleted Task:" = @("Action to perform when double-clicking an uncompleted download task.", "设置双击未完成下载任务时的行为。")
    ...
    "Default Checksum:" = @("New tasks no longer show checksum mode separately. This setting is used globally.", "新建任务中不再单独显示校验方式，统一使用这里的设置。")
    ...
    "Default User-Agent (empty for Chrome 154):" = @("New HTTP tasks prefill this UA, and each task can override it.", "新建 HTTP 任务会自动带入该 UA，也可以在任务里单独覆盖。")
    ...
    "Proxy Mode:" = @("Supports common HTTP / HTTPS / SOCKS proxy URLs. Enter a full URL.", "支持常见 HTTP / HTTPS / SOCKS 代理地址，按完整 URL 填写。")
    "Adaptive Profile:" = @("Conservative limits thread growth, Balanced trades overhead for speed, and Aggressive prioritizes download speed.", "保守更偏节制线程，平衡兼顾开销与速度，激进优先追求下载速度。")
    "Chunk Strategy:" = @("Auto-adjust chunk strategy based on network conditions", "根据网络状况自动调整分块策略")
    ...
    "Encryption Mode:" = @("MSE/PE protocol encryption. Forced mode may reject unencrypted connections. irontide only.", "MSE/PE 协议加密。强制模式可能拒绝未加密连接。仅 irontide。")
    "Pre-allocation Mode:" = @("Full preallocation reduces disk fragmentation but is slower on task creation. irontide only.", "完全预分配可减少磁盘碎片，但创建任务较慢。仅 irontide。")
    ...
    "Action:" = @("Ban disconnects and blacklists offending peers for a while; Limit Slots caps how many peers a torrent uploads to at once.", "封禁：断开并暂时拉黑违规对等端；限制上传槽位：当 torrent 有吸血对等端时限制同时上传的槽位数。")
    ...
    "Seed Choking:" = @("How peers are ranked while seeding. Anti-Leech prefers seeding peers that contribute the most back.", "做种时对 peer 的排序方式。反吸血优先回报最高的做种对等端。")
    "Choking Algorithm:" = @("How unchoke slots are determined per torrent. Fixed keeps a constant slot count; Rate-based auto-adjusts.", "每个 torrent 的 unchoke 槽位如何确定：固定槽位数 或 按速率自动调整。")
