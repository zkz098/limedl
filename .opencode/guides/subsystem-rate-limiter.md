...
HTTP 下载（http_executor.rs）
  ...
  ├─ 累计达到 ~256KB (BATCH_BYTES) 或 8 chunk (BATCH_CHUNKS, 取先到) 时调 rate_limiter.consume(accumulated).await
  ├─ loop 正常退出（stream end / chunk completed / cancellation）前 flush 剩余累加字节
...
- Clone 通过 Arc，零拷贝
- **批量 consume 优化**：http_executor 不再每 ~16KB chunk 调一次 consume，改为累积到 ~256KB 或 8 chunk（取先到）才调一次。loop 正常退出前 flush 剩余累加字节。stream error 路径未 flush（已知小遗漏，≤256KB 未核算流量，对速率限流精度影响微小）。AIMD 收敛不受影响（采样窗口 2s 远大于批量时间窗口）。
