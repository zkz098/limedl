...
use flareget_core::types::StartDownloadRequest;
use flareget_core::types::TaskId;
use flareget_core::{BackendRegistry, DownloadManager, EventBus};
...
async fn handle_settings_save(
    ...
    let settings: flareget_core::types::AppSettings =
...
async fn handle_fetch_tracker_list(
    ...
    let normalized = flareget_core::normalize_tracker_list_url(tracker_list_url)
    ...
    let content = String::from_utf8(response.to_vec())
        .map_err(|e| JsonRpcError::server_error(e.to_string()))?;
    let result = flareget_core::normalize_tracker_list_lossy(&content);
...
async fn handle_bt_runtime_status(
    ...
    let bt = state.registry.get_typed::<flareget_core::IrontideBtBackend>()
...
async fn handle_cdn_routes(
    ...
    match method {
        "cdn.fetchRanges" => {
            Ok(serde_json::json!(
                flareget_core::cdn::CLOUDFLARE_IPV4_RANGES.iter().map(|s| s.to_string()).collect::<Vec<_>>()
            ))
        }
        "cdn.status" => {
            let dm = state.registry.get_typed::<DownloadManager>().ok_or_else(|| {
...
mod tests {
    ...
    async fn make_rpc_state() -> (Arc<RpcState>, TempDir) {
        ...
        let core = flareget_core::bootstrap::bootstrap(state_dir).await.unwrap();
