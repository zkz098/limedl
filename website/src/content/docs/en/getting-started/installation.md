...
```powershell
Get-FileHash -Algorithm SHA256 .\limedl-native-v0.3.13-windows-x86_64-setup.exe
```
...
1. Download `limedl-native-v0.3.13-darwin-aarch64-portable.tar.gz` and extract it.
...
1. Download `limedl-native-v0.3.13-linux-x86_64.AppImage`.
2. Make it executable and run:

```bash
chmod +x limedl-native-v0.3.13-linux-x86_64.AppImage
./limedl-native-v0.3.13-linux-x86_64.AppImage
```
...
```bash
sudo apt install ./limedl-native-v0.3.13-linux-x86_64.deb
```

### Method 3: Portable Tarball (.tar.gz)

```bash
tar -xzf limedl-native-v0.3.13-linux-x86_64-portable.tar.gz
cd limedl-native
./limedl-native
```
