...
fn dirs_data_dir() -> PathBuf {
    ...
    if let Ok(dir) = std::env::var("FLAREGET_DATA_DIR") {
        return PathBuf::from(dir);
    }
    #[cfg(target_os = "linux")]
    {
        if let Ok(dir) = std::env::var("XDG_DATA_HOME") {
            return PathBuf::from(dir).join("flareget");
        }
    }
    let home = dirs_home();
    #[cfg(target_os = "linux")]
    {
        home.join(".local/share/flareget")
    }
    #[cfg(target_os = "macos")]
    {
        home.join("Library/Application Support/flareget")
    }
    #[cfg(target_os = "windows")]
    {
        home.join("AppData/Local/flareget")
    }
}
