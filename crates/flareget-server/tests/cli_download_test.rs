...
async fn cli_download_from_test_server() {
    ...
    let server = flareget_core::test_harness::TestServer::new(1024 * 1024).await;
    ...
    let task_id = flareget_core::types::TaskId::parse(&id);
    let inner = task_id.http_inner().unwrap();
    let start = std::time::Instant::now();
    loop {
        let snapshot = dm.status(inner).await.unwrap();
        if snapshot.state == flareget_core::types::DownloadState::Completed {
            break;
        }
        if snapshot.state == flareget_core::types::DownloadState::Failed {
    ...
    let hash = flareget_core::checksum::hash_slices(
        flareget_core::types::ChecksumMode::Blake3,
        slices,
    );
    assert_eq!(
