...
import SettingsAria2RpcPanel from "../../../components/settings/SettingsAria2RpcPanel.vue";
...
describe("SettingsAria2RpcPanel", () => {
  it("renders section header and port field", () => {
    const draft = reactive(createSettings());
    const wrapper = mount(SettingsAria2RpcPanel, {
      props: {
        draft,
        t: (key: string) => key,
      },
    ...
    expect(wrapper.text()).toContain("settings.aria2RpcTitle");

    // Port field label renders
    expect(wrapper.text()).toContain("settings.aria2RpcPort");

    // Secret field exists
    expect(wrapper.text()).toContain("settings.aria2RpcSecret");

    // Service switch exists
    expect(wrapper.text()).toContain("settings.aria2RpcService");

    // Input fields are rendered
    expect(wrapper.find(".ui-textfield-stub").exists()).toBe(true);
