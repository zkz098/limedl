...
// ===========================================================================
// Test 1: Traditional mode limits concurrent tasks
// ===========================================================================

/// Start 3 downloads with max_parallel=2 and a slow server (1 s startup delay).
/// The first two should be running, the third queued.
#[tokio::test]
#[timeout(180_000)]
async fn traditional_mode_limits_concurrent_tasks() -> TestResult {
    let server = TestServer::new(10 * 1024 * 1024).await; // 10 MB
    let url = server.file_url_slow(2000); // 2 s startup delay, single-stream
    ...
    let id1 = manager.start(req_fixed(&url, &out, "one.bin")).await?;
    ...
    assert_eq!(
        ...
        "expected Downloading, got {s1:?}"
    );
    assert_eq!(
        s2.state,
        DownloadState::Downloading,
        "expected Downloading, got {s2:?}"
    );
    assert_eq!(
        s3.state,
        DownloadState::Queued,
        "expected Queued, got {s3:?}"
    );
...
// ===========================================================================
// Test 6: Completion triggers next
// ===========================================================================

/// Start 2 downloads with max_parallel=1.  Verify the second is queued,
/// the first completes, then rebalance starts the second.
#[tokio::test]
#[timeout(180_000)]
async fn scheduler_handles_completion_and_starts_next() -> TestResult {
    ...
    let id1 = manager.start(req_fixed(&url, &out, "first.bin")).await?;
